...
export const deleteProduct = async (req, res) => {
  ...
  try {
    if (req.user.role === "user") {
      return res
        .status(401)
        .send("No tienes permisos para eliminar un producto");
    }
    ...
    const ownerProduct = await usersServices.getUserById(deleteProduct.owner);
    // Si el user del producto que se elimino es de rol premium se le envia un mail avisando que producto y cual era el id del mismo
    if (ownerProduct.role === "premium") {
      const mailOptions = {
        from: "Coder-E-Comerce",
        to: ownerProduct.email,
        subject: "Informacion sobre E-Commerce-Coder",
        text: `El producto ${deleteProduct.title} con el ID:${deleteProduct._id} a sido eliminado`,
      };
